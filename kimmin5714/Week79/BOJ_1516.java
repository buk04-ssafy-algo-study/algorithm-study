...
public class BOJ_1516 {
    public static void main(String[] args) throws IOException {
        ...
        int[] resultTime = new int[N+1]; // 선행 건물 짓는데 걸리는 시간
        ...
        while(!q.isEmpty()) {
            ...
            for(int prev : adj.get(cur)) { // 인접한 노드들 진입차수 감소
                indegree[prev]--;

                // 가장 오래 걸리는 선행 건물이 끝난 다음에 시작할 수 있음
                resultTime[prev] = Math.max(resultTime[prev], resultTime[cur] + buildTime[prev]);

                if(indegree[prev] == 0) { // 진입차수가 0이면 큐에 추가
                    q.offer(prev);
                }
            }
        }
